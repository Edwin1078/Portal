...
def get_oracle_connection():
    ...
    if not os.path.exists(env_path):
        # Intentar ruta relativa al archivo si el CWD falla
        env_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '.env')
    
    load_dotenv(env_path, override=True) # Override para asegurar que tome los nuevos
    
    config = settings.ORACLE_DB_CONFIG
    try:
        # Priorizar variables de entorno para mayor flexibilidad
        ...
        port = os.getenv('DB_PORT') or os.getenv('DB_PORT_ORACLE') or config.get('PORT')
        name = os.getenv('DB_NAME') or config.get('NAME')
        
        with open("debug_db.txt", "a") as f:
            f.write(f"PARAMETROS DETECTADOS: User={user}, Host={host}, Port={port}, DB={name}\n")
        ...
        with open("debug_db.txt", "a") as f:
            f.write(f"ERROR CONEXION ({pd.Timestamp.now()}): {str(e)}\n")
        return None
...
def execute_query(query, params=None):
    ...
    try:
        ...
        if conn: conn.close()
        with open("debug_db.txt", "a") as f:
            f.write(f"ERROR EXECUTE ({pd.Timestamp.now()}): {str(e)}\n")
            f.write(f"QUERY SAMPLE: {query[:200]}...\n")
        return None
...
def get_global_stats():
    ...
    try:
        ...
        # 3. Suma total de pagos (CON_PAGO)
...
def get_all_periods():
    # Cache por 1 hora (los periodos no cambian frecuentemente)
    ...
    try:
        ...
        if conn: conn.close()
        with open("debug_db.txt", "a") as f:
            f.write(f"ERROR PERIODOS ({pd.Timestamp.now()}): {str(e)}\n")
        return []
...
def get_account_history(account):
    query = "SELECT * FROM DATABOOST_PROD.HDB_DATOS_COMERCIALES WHERE CUENTA = :acc ORDER BY PERIODO DESC"
